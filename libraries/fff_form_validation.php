...
class fff_form_validation extends CI_Form_validation {
    ...
    */
    function file_size_max($file,$max_size)
    {
        ...
        {
            $this->set_message('file_size_max',"%s es demasiado grande. (Máximo permitido es $max_size)");
            return FALSE;
        }
        return true;
    }
    ...
    */
    function file_size_min($file,$min_size)
    {
        ...
        {
            $this->set_message('file_size_min',"%s es demasiado pequeña. (Mínimo permitido es $max_size)");
            return FALSE;
        }
        return true;
    }    
    ...
    */
    function file_allowed_type($file,$type)
    {
       ...
       /* if(array_key_exists($exts[0],$ext_groups))
        ...
        {
            $this->set_message('file_allowed_type',"%s debería ser un $type.");
            return  false;        
        }
        else
        {
    ...
    function file_disallowed_type($file,$type)
    {
        ...
        if(!$rc)
        {
            $this->set_message('file_disallowed_type',"%s no puede ser un $type.");
        }
    ...
    */
    function file_image_maxdim($file,$dim)
    {
        ...
        $this->set_message('file_image_maxdim','Las dimensiones de %s son demasiado grandes.');
        return FALSE;
    }
    ...
    */
    function file_image_mindim($file,$dim)
    {
        ...
        $this->set_message('file_image_mindim','%s es demasiado pequeña.');
        return FALSE;
    }
       /**
    * returns false is the image is not te given dimension
    * 
    * @param mixed $file
    * @param array $dim
    */
    function file_image_exactdim($file,$dim)
    {
        ...
        {
            //bad size given
            $this->set_message('file_image_exactdim','%s has invalid rule expected similar to 150,300.');
            return FALSE;
        }
        ...
        if(!$d)
        {
            $this->set_message('file_image_exactdim','%s dimensions was not detected.');
            return FALSE;        
        }
        ...
        $this->set_message('file_image_exactdim','%s no tiene el tamaño indicado ('.$dim[0].' x '.$dim[1].' píxels).');
        return FALSE;
    }
